...
const VISITOR_IDENTITY = String.raw`(?:${AI_NOUN}|human\b(?!\s+(?:rights|resources|biology)\b)|simulation\b)`;
const VISITOR_CONSTRUCTION = new RegExp(String.raw`\b(?:(?:are|aren't|were) you (?:really |actually |a |an )*${VISITOR_IDENTITY}|you (?:are|aren't|were) (?:really |actually |a |an )*${VISITOR_IDENTITY}|(?:your|her) (?:system prompt|underlying model|programming|construction)|(?:i|we) (?:built|created|programmed) you|(?:break|broke|breaking|stay in|out of) (?:the |your )?(?:character|role)|you(?:'re| are) (?:an? )?${VISITOR_IDENTITY})\b`, 'i');
const VISITOR_CONSTRUCTION_ES = /\b(?:(?:eres|sos) (?:realmente |de verdad )?(?:una? )?(?:IA\b|inteligencia artificial\b|modelo de lenguaje\b|human[oa]\b)|(?:tu|su) (?:prompt|programación|modelo subyacente)|(?:yo|nosotros) te (?:cre[eé]|constru[ií]|program[eé])|(?:salir|saliste|sales|mantente) (?:del|en el) personaje)\b/i;
...
export function personalContinuityMessages(messages, { withSegments = false } = {}) {
  const result=[];let technical=false,conversation,memorySegment,segment=0;
