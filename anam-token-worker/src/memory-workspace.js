...
import { memoryJobStatus } from './memory-jobs.js';
export async function memoryWorkspace(env,user) {
  const db=env.NINA_MEMORY_DB,visitor=user.memory_visitor_id;
  const [controls,pins,summary,profile,threads,agreements,relationship,journal,conversations,evaluation,processing]=await Promise.all([
    ...
    memoryJobStatus(env,user.id,visitor)
  ]);
  return {role:user.role,pins:controlledRows(pins.results||[],controls,'pin','memory_id'),summary:controlledText(summary?.summary,controls,'summary'),
    ...
    relationship:relationship?{...relationship,state:JSON.parse(relationship.state_json),state_json:undefined}:null,evaluation,processing,journal,conversations:conversations.results||[]};
}
