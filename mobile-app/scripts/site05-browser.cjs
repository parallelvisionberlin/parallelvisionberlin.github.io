...
(async()=>{
 ...
 const server=http.createServer((req,res)=>{const p=path.join(out,decodeURIComponent(new URL(req.url,'http://localhost').pathname));const file=fs.existsSync(p)&&fs.statSync(p).isFile()?p:path.join(out,'index.html');const ext=path.extname(file);res.setHeader('Content-Type',({'.js':'text/javascript','.jpg':'image/jpeg','.png':'image/png','.mp4':'video/mp4','.html':'text/html'})[ext]||'application/octet-stream');res.end(fs.readFileSync(file));});
   ...
   try{
    for(const [width,height] of [[390,740],[320,568]]){
     ...
     for(const tab of ['HOME','NINA','2063','MUSIC','PROFILE']){
      ...
      if(tab==='HOME'||tab==='NINA')await page.waitForFunction(()=>{const v=document.querySelector('video');return v&&v.readyState>=2&&v.currentTime>0;},{timeout:12000});
