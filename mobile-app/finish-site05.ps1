...
$SourceCommit = '85f193bbef58ccd603082a853fbaba22fffbd0ff'
$Revision = 'SITE COHESION / 05.2'
...
try {
    ...
    $Name = 'SITE052_' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '_' + ([guid]::NewGuid().ToString('N').Substring(0, 6))
    ...
    if (![IO.File]::ReadAllText((Join-Path $App 'src\site05Model.js')).Contains($Revision)) { throw 'SITE 05.2 marker is missing.' }
    ...
    Write-Host "$Revision : checking hero playback fallback, account actions and iOS export." -ForegroundColor Cyan
