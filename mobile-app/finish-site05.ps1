...
$SourceCommit = '128a274fa2531790edaef9f8f774f651151322f4'
$Revision = 'SITE COHESION / 05.1'
...
try {
    ...
    $Name = 'SITE05_' + (Get-Date -Format 'yyyyMMdd_HHmmss') + '_' + ([guid]::NewGuid().ToString('N').Substring(0, 6))
    ...
    if (![IO.File]::ReadAllText((Join-Path $App 'src\site05Model.js')).Contains($Revision)) { throw 'SITE 05.1 marker is missing.' }
    ...
    Write-Host "$Revision : checking original films, native actions and iOS export." -ForegroundColor Cyan
    ...
    Write-Host 'Local source files, Cloudflare Worker and Anam settings were not changed.'
    if (!$CheckOnly) {
