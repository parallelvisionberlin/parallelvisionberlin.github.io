...
// ---- Worker analytics ----
{
  const path='anam-token-worker/src/analytics.js';
  let s=read(path);
  s=replaceOnce(s,
...
`;
  s=replaceOnce(s,`export async function getNinaAnalyticsDashboard(env, now = Date.now()) {`,detailFn+`\nexport async function getNinaAnalyticsDashboard(env, now = Date.now()) {`,'session detail export');
...
// ---- Worker route ----
{
  const path='anam-token-worker/src/index.js';
  let s=read(path);
  s=replaceOnce(s,
`  endNinaAnalyticsSession, getNinaAnalyticsDashboard, startNinaAnalyticsSession, touchNinaAnalyticsSession`,
`  endNinaAnalyticsSession, getNinaAnalyticsDashboard, getNinaAnalyticsSessionDetail, startNinaAnalyticsSession, touchNinaAnalyticsSession`,'analytics import');
  s=replaceOnce(s,
...
// ---- Admin HTML ----
{
  const path='nina-admin/index.html';
  let s=read(path);
  s=replaceOnce(s,
`<link rel="stylesheet" href="../css/nina-admin.css?v=20260904-readable-2">`,
`<link rel="stylesheet" href="../css/nina-admin.css?v=20260909-call-inspector">`,'admin css cache');
  s=replaceOnce(s,
`<div class="admin-section-heading"><p>07</p><h2 id="sessionsTitle">Recent sessions</h2></div>`,
`<div class="admin-section-heading"><p>07</p><div><h2 id="sessionsTitle">Recent sessions</h2><p class="admin-section-kicker">Click a call to inspect the conversation, credits and purchase path.</p></div></div>`,'session heading');
  s=replaceOnce(s,
`<script type="module" src="../js/nina-admin.js?v=20260905-voucher-copy"></script>`,
`<script type="module" src="../js/nina-admin.js?v=20260909-call-inspector"></script>`,'admin js cache');
  write(path,s);
}

// ---- Admin JS ----
{
  const path='js/nina-admin.js';
  let s=read(path);
  s=replaceOnce(s,
...
`;
  s=replaceOnce(s,`function renderSessions(sessions) {`,helper+`\nfunction renderSessions(sessions) {`,'detail helpers');

  const oldLoop=`    const row = document.createElement("tr");
    const identity = document.createElement("td");`;
  const newLoop=`    const row = document.createElement("tr");
    row.className = "admin-session-row";
    row.tabIndex = 0;
    row.setAttribute("role", "button");
    row.setAttribute("aria-expanded", expandedSessionId === session.id ? "true" : "false");
    const identity = document.createElement("td");`;
  s=replaceOnce(s,oldLoop,newLoop,'clickable session row');
...
`    elements.generated.textContent = \`Generated ${'${dateTime(data.generatedAt)}'} · Today resets at 00:00 Europe/Berlin · Active window ${'${data.activeWindowSeconds}'} seconds\`;`,'generated timezone label');
  write(path,s);
}

// ---- Admin CSS ----
{
  const path='css/nina-admin.css';
  let s=read(path);
  s += `
...
// ---- Focused tests ----
{
  const path='anam-token-worker/test/admin-analytics-fix.test.js';
...
console.log('Applied Nina admin analytics fix.');
