 ...
 async with async_playwright() as p:
  for name in ['chromium','webkit']:
   ...
   for width,height in [(390,740),(320,520),(430,800)]:
    ...
    html='<html><head><style>html,body{margin:0;height:100%;background:#030404}#ninaOverlay,.nina-stage{position:absolute;inset:0}button{font-size:16px}'+CSS+'</style></head><body><div id="ninaOverlay"><div class="nina-stage"><video id="nina-anam-video"></video><button id="startNina">Start</button></div></div></body></html>'
    ...
    await page.evaluate('''() => {window.captureCalls=0;window.stopCalls=0;window.audioCalls=0;window.track={readyState:'live',enabled:true,muted:false,stop(){window.stopCalls++}};window.stream={getAudioTracks(){return [window.track]}};navigator.mediaDevices.getUserMedia=()=>{window.captureCalls++;throw Error('Unexpected capture')};window.AudioContext=class{constructor(){window.audioCalls++;this.state='running'}createMediaStreamSource(){return {connect(){},disconnect(){}}}createAnalyser(){return {fftSize:256,getFloatTimeDomainData(a){a.fill(.08)},disconnect(){}}}close(){return Promise.resolve()}};}''')
    ...
    # A failed level-meter must not mute or stop the existing stream.
    ...
    await page.evaluate('deck.dispose()')
    results.append({'engine':name,'viewport':[width,height],'checks':['off-before-call','mute','unmute','accessible-state','fits-screen','no-capture','no-stop','meter-failure-safe','error-ui-clear','cleanup']})
    await page.close()
   await browser.close()
